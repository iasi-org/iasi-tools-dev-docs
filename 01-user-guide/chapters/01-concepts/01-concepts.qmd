...
Cuando existe `publish/`, `deploy` separa los cambios ordinarios y los artefactos publicables en commits distintos antes de realizar `push`.
...
| Quiero… | Operación |
|---|---|
| preparar por primera vez todo el entorno IASI | `init` |
| recuperar todos los repositorios desde cero | `clone` |
| añadir únicamente los repositorios que faltan | `clone --resume` |
| sustituir el estado local por el remoto | `pull` |
| distribuir un recurso de `iasi-common` | `sync` |
| generar resultados revisables | `build` |
| preparar artefactos en `publish/` | `publish` |
| confirmar y enviar el estado ya preparado | `deploy` |
| construir, publicar y desplegar | `deploy --full` |
| confirmar y enviar todos los cambios sin tratamiento documental | `commit` |
| gestionar los servicios locales | `docker` |
