...
`commit` no construye ni prepara publicaciones y no da un tratamiento separado a `publish/`. Para desplegar documentación resulta más adecuado `deploy`.
...
Despliega los cambios mediante commits y `push`. Cuando existe `publish/`, separa los cambios ordinarios y los artefactos publicados en commits distintos.
